...
module.exports = async function () {
	...
	if (dev > 0) {
		...
		const devTools = await require("./dev")(
			require(path.join(process.cwd(), appConfigLocation, "client.config.js")),
			require(path.join(process.cwd(), appConfigLocation, "server.config.js")),
		);

		devTools.setupDevServer(app);
		setUp(app);
		app.use(require("./vuessr")({ renderFunctionFactory: devTools.createRenderFunction }));
	} else {

...
module.exports = async function (server, options) {
	...
	if (dev > 0) {
		...
		const { middleware, renderFunctionFactory } = await require("./dev")({
			clientConfig: path.resolve(appConfigLocation, "client.config.js"),
			serverConfig: path.resolve(appConfigLocation, "server.config.js"),
		...
		setupBasicMiddlewares(app, options);
		app.use(ssr(options));
		app.use(ctx => send(ctx, "index.html", { root: options.contentRoot, maxage: options.cacheMaxAge }));
	}
